...
즉, “문자열을 구조화하고”, “구조에 스타일을 입히고”, “화면 좌표를 계산한 뒤”, “픽셀로 그린다”는 흐름이다.
...
핵심 포인트:
- DOM tree를 순회하며 `<link rel="stylesheet">`, `<img src>`를 찾는다
- `net` 모듈은 “한 URL을 가져오는 역할”
- `resource` 모듈은 “DOM에서 필요한 URL들을 모으는 역할”
...
`문자열(HTML/CSS)을 트리로 바꾸고 -> 트리에 스타일을 입히고 -> 박스 좌표를 계산하고 -> 화면에 그리는 작은 브라우저`
