// Text measurement + glyph rasterisation. The top of the file is the
// measurement API the layout / display-list pipelines call (`measure_*`)
// to size content boxes correctly; the bottom is the paint path swung
// in from `rasterize_command` via the shared `draw_text` entry point.
//
// Two glyph paths run side by side:
//   - cosmic-text shaping + swash rasterisation (the production path,
//     only active once `state::install_fonts` has populated the shared
//     `FontSystem` / `SwashCache` slots)
//   - a deterministic 7×7 bitmap fallback (`draw_text_bitmap`) for the
//     test surface, so pixel-pinned assertions don't depend on host
//     fonts
...
use super::super::{Affine, TextCommand};
use super::blend::{blend_pixel_bytes, paint_through};
use super::shapes::fill_solid_rect;
...
pub(super) fn draw_text(pixmap: &mut Pixmap, text: &TextCommand, transform: Affine) {
